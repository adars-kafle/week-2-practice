...
class Resource {
    constructor(name, unitCost, quantity, duration) {
        this.name = name;
...
class Task {
    // Constructor of Task
    constructor(id, name, wbsId, claimRef, contractSum, margin) {
        this.resources = null;
        this.tasks = null;
        this.id = id;
        this.name = name;
    ...
    getBudget() {
        let totalBudget = 0;
        if (this.resources) {
            totalBudget += this.resources.reduce((total, resource) => total + resource.getBudget(), 0);
        }
        if (this.tasks) {
            totalBudget += this.tasks.reduce((total, task) => total + task.getBudget(), 0);
        }
        return totalBudget;
    }
    toJSON() {
...
// Main project task
const buildBuilding = new Task(1, "Build a Building", "WBS001", "CR001", 1000000, 20);
// Subtasks
